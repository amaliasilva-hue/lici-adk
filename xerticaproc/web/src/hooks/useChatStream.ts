...
interface UseChatStreamOpts {
  onChecklistUpdated?: () => void;
  onTurnComplete?: (mensagemId: string) => void;
}
...
export function useChatStream(contratacaoId: string, opts: UseChatStreamOpts = {}) {
  ...
  React.useEffect(() => {
    ...
    setError(null);
    getHistory(contratacaoId, { limit: 50 })
      .then((h: ChatHistoryResponse) => {
        if (!cancelled) setMessages(h.mensagens);
      })
      .catch(e => {
  ...
  const send = React.useCallback(
    async (text: string) => {
      ...
      const onEvent = (ev: StreamEvent) => {
        if (ev.event === "assistant_token") {
          draftRef.current += ev.data.delta;
          const draft = draftRef.current;
          setMessages(prev =>
            prev.map(m => (m.id === assistantId ? { ...m, conteudo: draft } : m)),
          );
          ...
          setMessages(prev =>
            prev.map(m =>
              m.id === assistantId
                ? { ...m, id: ev.data.mensagem_id, conteudo: finalContent, intencao: ev.data.intencao ?? null }
                : m,
            ),
          );
          opts.onTurnComplete?.(ev.data.mensagem_id);
        } else if (ev.event === "error") {
          setError(ev.data.message);
        }
      };
  ...
  return { messages, pending, error, send, cancel };
}
