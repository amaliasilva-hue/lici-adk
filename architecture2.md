...
5. **`ResumoExecutivo`** — `conformidade_geral` · `score_conformidade` (0–100) · `pontos_criticos` · `recomendacao` ("participar" / "impugnar antes" / "aguardar retificação") · `proximos_passos`
...
**Não é kanban genérico.** É um pipeline de licitação com stages tipados, gates por checklist e estados terminais. x-lici termina em `homologado` — o que vem depois (empenho, entrega, contrato) vai para o SaaS de contratos (Fase 10).
...
calculado_em  TIMESTAMPTZ NOT NULL DEFAULT now()
  -- invalidação: Cloud Scheduler job detecta arquivo novo no Drive
  --   → DELETE FROM atestados_cache WHERE edital_id = ?
```
...
| `DELETE` | `/editais/{id}` | Soft delete (seta `deleted_at`; registro preservado no BQ via Datastream) |
...
**Auth:** todos exceto `/healthz` exigem:
- **SA ID token** do frontend — verificado pelo IAM do Cloud Run; autentica que a requisição vem de `x-lici-web` (não forjável por um cliente externo)
- **Header `X-User-Email`** — preenchido pelo SSR a partir do JWT NextAuth, nunca exposto diretamente pelo cliente; validado como `@xertica.com` + allowlist. O header sozinho seria forjável, mas a combinação com o SA ID token garante que só `x-lici-web` pode enviar requisições
...
- [ ] Cache `atestados_cache` em Postgres (invalidado pelo Cloud Scheduler job)
- [ ] Cloud Scheduler job a cada 15 min: re-scan Drive por timestamp → `DELETE FROM atestados_cache WHERE edital_id = ?` se arquivo novo detectado + notifica edital
...
- [ ] `backend/agents/analista_licitatorio.py` — 6 blocos: FichaProcesso + AtestadoAnalise + RiscoJuridico + DocumentosProtocolo (**ESCLARECIMENTO** | **IMPUGNACAO** com prazos calculados) + ResumoExecutivo + KitHabilitacao
...
9. **Padronizar estrutura de pastas no Drive** (subpastas com nomes exatos: `Edital`, `Atestados`, `Habilitação`, `Proposta`, `Contrato`). `Atestados/` é crítica.
10. **Definir lista de emails autorizados** (jurídico + vendedores + diretoria) → allowlist de auth
...
6. ✅ Fase 4 — somador de atestados + Drive API read-only + Cloud Scheduler
