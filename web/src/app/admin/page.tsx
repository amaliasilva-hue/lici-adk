...
export default function AdminPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="text-slate-500 text-sm py-8 text-center">Carregando métricas…</div>
      ) : (
        <>
          {/* Stats grid */}
          ...
          {/* Stage breakdown */}
          <div className="card">
            <h2 className="font-semibold text-slate-700 mb-4 text-sm">Distribuição por fase</h2>
            <div className="space-y-2">
              {Object.entries(byStage)
                .sort((a, b) => b[1] - a[1])
                .map(([stage, count]) => (
                  <div key={stage} className="flex items-center gap-3">
                    <span className="w-32 text-xs text-slate-500 capitalize">{stage.replace('_', ' ')}</span>
              ...
              {Object.keys(byStage).length === 0 && (
                <p className="text-slate-400 text-sm">Nenhum edital ativo.</p>
              )}
            </div>
          </div>
